 ...
 * modifyer hojk 2018-03-13 Email : hojk@foxmail.com
*/
...
var ZYFILE = {
	...
	funGetLength : function(){
		var fileLength = this.uploadFile.length;
		return fileLength;
	},
	// 获取文件
...
//					edit             : true,  						      // 是否可以编辑文件（裁剪）
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
		            ...
		            html += '				</div>';
		            html += '			</div>';
					html += '           <div class="errorMsgBox"></div>';
		            ...
		            html += '			</div>';
					html += '           <div class="errorMsgBox"></div>';
		            html += '			<div id="preview_complete" class="upload_preview">';
				    html += '				<div class="add_upload_complete">';
				    html += '				</div>';
					html += '			</div>';
				    ...
				    html += '							<img class="upload_image" src="/Public/js/zyupload/skins/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>'; 
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
...
//								}
					...
					onSuccess: function(file, response) {
						...
						// hojk
						self.createAlreadyFilesHtml(retData.data,file.index);
					},
			...
			/**
			 * by hojk
			 * used give already upload files bind to page
			 * 2018-03-16
			 */
			this.createAlreadyFiles = function(){
				var ids = options.already;
				if(ids != ""){
					var splitIds = ids.split(',');
					$.each(splitIds,function(k,v){
						$.get(options.getUrl+"?id="+v,function(ret){
							if(ret){
								var $index = v + 10000;
								var $data = {"id":v,"url":ret};
								self.createAlreadyFilesHtml($data,$index);
							}
						});
					});
				}
			};
			this.createAlreadyFilesHtml = function(data,index){
				var imgWidth = parseInt(para.itemWidth.replace("px", ""))-15;
				var imgHeight = parseInt(para.itemHeight.replace("px", ""))-10;
				var html = '';
				html += '<div id="uploadList_c_'+ index +'" class="upload_append_list">';
				html += 			'<span class="file_del_complete" data-id="'+data.id+'" data-index="'+index+'" title="删除"></span>';
				html += '	<a style="height:'+para.itemHeight+';width:'+para.itemWidth+';" href="#" class="imgBox">';
				html += '		<div class="uploadImg" style="width:'+imgWidth+'px">';				
				html += '			<img id="uploadImage_'+index+'" class="upload_file" data-id="'+data.id+'" src="' + data.url.slice(1) + '" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';                                                                 
				html += '		</div>';
				html += '	</a>';
				html += '</div>';
				var funAppendPreviewCompleteHtml = function(html){
					// 添加到添加按钮前
					if(para.dragDrop){
						$("#preview_complete").append(html);
					}else{
						$(".add_upload_complete").before(html);
					}
					// 绑定删除按钮
				};
				funAppendPreviewCompleteHtml(html);
				$(".file_del_complete").click(function() {
					var $this = $(this);
					var $id = $this.attr('data-id');
					var $url = options.delUrl + "?id=" + $id;
					alertMsg.confirm("确定要删除已上传的文件吗？", {
                        okCall: function(){
                        	$this.closest(".upload_append_list").remove();
                        	var xhr = new XMLHttpRequest();
			...
			this.addEvent = function(){
				 ...
				 * 2018-03-13
				 */
				var checkUploadFilesIfExceedLimit = function(){
					var lmt = options.fileLimit;
					var nowLmt = ZYFILE.funGetLength();
					var has = getAlreadyIds("len");
					var counts = nowLmt + has;
					if(lmt && counts < lmt){
						$("#fileImage").click();
					}else{
						msgBox(false,"超出上传限制："+lmt);
					}
				};
				// 如果快捷添加文件按钮存在
 ...
 * 2018-03-13
 */
var msgBox = function(status,msg){
	var $mBox = $(".errorMsgBox");
	if(status){
		$mBox.css("background","green").fadeIn();
		setTimeout(function(){
			$mBox.fadeOut();
		},3000);
	}else{
		$mBox.css("background","red").fadeIn();
		setTimeout(function(){
			$mBox.fadeOut();
		},3000);
	}
 ...
 * 2018-03-16
 */
var getAlreadyIds = function(type = "ids"){
	var $fdc = $('.file_del_complete');
	if(type == "ids"){
		var $arr = [];
		$fdc.each(function(){
