// You are licensed to use this file as described in the evaluation license text
// you agreed to when downloading the mxGraph evaluation package.
// Note that you are NOT licensed to make a copy of this file on any file
// system. See the evaluation license for full details.
// File created 30-Apr-2010 05:28:55 for Mozilla/5.0 (Windows; U; Windows NT
// 5.2; zh-CN; rv:1.9.2.2) Gecko/20100316 Firefox/3.6.2 QQDownload/1.7
var mxClient = {
	VERSION : '1.4.0.0',
...
var mxUtils = {
	...
	sortCells : function(cells, ascending) {
		...
		cells.sort(function(o1, o2) {
					var p1 = mxCellPath.create(o1)
							.split(mxCellPath.PATH_SEPARATOR);
					var p2 = mxCellPath.create(o2)
							.split(mxCellPath.PATH_SEPARATOR);
					var comp = mxCellPath.compare(p1, p2);
					return (comp == 0) ? 0 : (((comp > 0) == ascending)
							? 1
							: -1);
				});
		return cells;
		...
		mxEvent.addListener(this.div, 'DOMNodeInserted', mxUtils.bind(this,
						function(evt) {
							...
							var loadHandler = mxUtils.bind(this, function(evt) {
										mxEvent.removeListener(node, 'load',
												loadHandler);
										this.updateShadow();
									});
...
mxWindow.prototype.installMinimizeHandler = function() {
	...
	var funct = mxUtils.bind(this, function(evt) {
				...
				if (!minimized) {
					...
					this.updateShadow();
					this.fireEvent(new mxEventObject(mxEvent.MINIMIZE, 'event',
							evt));
				} else {
					minimized = false;
					this.minimize.setAttribute('src', this.minimizeImage);
	...
	mxEvent.addListener(this.title, 'mousedown', mxUtils.bind(this, function(
						...
						var dropHandler = mxUtils.bind(this, function(evt) {
									...
									mxEvent.removeListener(document, 'mouseup',
											dropHandler);
									this.fireEvent(new mxEventObject(
											mxEvent.MOVE_END, 'event', evt));
									mxEvent.consume(evt);
								});
						mxEvent.addListener(document, 'mousemove', dragHandler);
						mxEvent.addListener(document, 'mouseup', dropHandler);
						this.fireEvent(new mxEventObject(mxEvent.MOVE_START,
								'event', evt));
						mxEvent.consume(evt);
					}));
};
...
mxPath.prototype.quadTo = function(x1, y1, x, y) {
	if (this.isVml()) {
		this.path.push('qb ', Math.floor(this.translate.x + x1), ' ', Math
						.floor(this.translate.y + y1), ' ', Math
						.floor(this.translate.x + x), ' ', Math
						.floor(this.translate.y + y), ' l ', Math
						.floor(this.translate.x + x), ' ', Math
						.floor(this.translate.y + y), ' ');
	} else {
		this.path.push('Q ', (this.translate.x + x1), ' ',
				(this.translate.y + y1), ' ', (this.translate.x + x), ' ',
				(this.translate.y + y), ' ');
	}
};
...
mxShape.prototype.updateSvgTransform = function(node, shadow) {
	if (this.rotation != null && this.rotation != 0) {
		...
		if (shadow) {
			node
					.setAttribute('transform', 'rotate(' + this.rotation + ','
									+ cx + ',' + cy + ') '
...
mxArrow.prototype.redrawVml = function() {
	...
	if (this.points != null) {
		...
		var d = 'm' + Math.round(p0x) + ',' + Math.round(p0y) + ' l'
...
mxText.prototype.updateHtmlTable = function(table, scale) {
	...
	if (false) {
		if (this.isStyleSet(mxConstants.FONT_SHADOW)) {
			td.style.filter = 'Shadow(Color=#666666,'
	...
	if (this.overflow != 'fill') {
		...
		if (this.clipped && this.bounds.width > 0 && this.bounds.height > 0) {
			...
			if (this.horizontal || (oldBrowser && !mxClient.IS_OT)) {
				...
				table.style.clip = 'rect(' + (dy) + ' '
						+ (dx + this.bounds.width * s2) + ' '
						+ (dy + this.bounds.height * s2) + ' ' + (dx) + ')';
			} else {
				if (false) {
					...
					table.style.clip = 'rect(' + (dx) + ' ' + (dy + uw - 1)
							+ ' ' + (dx + uh - 1) + ' ' + (dy) + ')';
				} else {
			...
			this.boundingBox = new mxRectangle(x * this.scale, y * this.scale,
	...
	if (this.overflow != 'fill') {
		...
		if (this.clipped && this.bounds.width > 0 && this.bounds.height > 0) {
			...
			if (this.backgroundNode != null) {
				...
				this.backgroundNode.setAttribute('clip-path', 'url(#'
...
mxText.prototype.redrawSvg = function() {
	...
	if (this.value.length > 0 && this.backgroundNode != null
		...
		this.backgroundNode.setAttribute('x', this.boundingBox.x + 2
						* this.scale);
		this.backgroundNode.setAttribute('y', this.boundingBox.y + 2
						* this.scale);
...
mxLine.prototype.redrawVml = function() {
	...
	if (this.direction == mxConstants.DIRECTION_NORTH
			|| this.direction == mxConstants.DIRECTION_SOUTH) {
		this.innerNode.setAttribute('path', 'm ' + (w / 2 - x) + ' ' + (-y)
						+ ' l ' + (w / 2 - x) + ' ' + (h - y) + ' e');
	} else {
		this.innerNode.setAttribute('path', 'm ' + (-x) + ' ' + (h / 2 - y)
						+ ' l ' + (w - x) + ' ' + (h / 2 - y) + ' e');
	}
};
...
mxLabel.prototype.redraw = function() {
	...
	if (this.indicator != null) {
		this.indicator.bounds = new mxRectangle(x + (width - indicatorWidth)
						/ 2, y + imageHeight + indicatorSpacing,
...
mxGraphLayout.prototype.getVertexBounds = function(cell) {
	...
	if (this.useBoundingBox) {
		...
		if (state != null && state.text != null
			...
			var dx1 = Math
					.max((tmp.x + tmp.width) - (state.x + state.width), 0)
					/ scale;
			var dy1 = Math.max((tmp.y + tmp.height) - (state.y + state.height),
					0)
					/ scale;
	...
	if (node.cell != null && g != null) {
		...
		if (bounds == null) {
			...
			bounds = new mxRectangle(Math.min(bounds.x, g.x), Math.min(
							bounds.y, g.y), Math.max(bounds.x + bounds.width,
							g.x + g.width), Math.max(bounds.y + bounds.height,
							g.y + g.height));
		}
	}
	return bounds;
		...
		for (var i = 0; i < layerDeterminingEdges.length; i++) {
			...
			if (internalEdge.temp[0] == 5270620) {
				...
				if (this.scanRanksFromSinks) {
					...
					minimumLayer = Math
							.min(minimumLayer, otherNode.temp[0] - 1);
				}
			} else {
...
mxHierarchicalLayout.prototype.run = function(parent) {
	...
	for (var i = 0; i < hierarchyVertices.length; i++) {
		...
		this.model = new mxGraphHierarchyModel(this, tmp, this.roots, parent,
				false, this.deterministic, this.tightenToSource, true);
...
mxPrintPreview.prototype.addPageClickListener = function(cell, pageNumber) {
	mxEvent.addListener(cell, 'click', mxUtils.bind(this, function(evt) {
						var page = this.wnd.document.getElementById('mxPage-'
								+ pageNumber);
						if (page != null) {
							this.wnd.scrollTo(0, Math
											.max(0, page.offsetTop - 8));
						}
					}));
};
...
mxCellState.prototype.setCursor = function(cursor) {
	if (this.shape.innerNode != null
			&& !this.view.graph.getModel().isEdge(this.cell)) {
...
mxCellEditor.prototype.startEditing = function(cell, trigger) {
	...
	if (state != null) {
		...
		var size = mxUtils.getValue(state.style, mxConstants.STYLE_FONTSIZE,
				...
				* scale;
		var family = mxUtils.getValue(state.style,
				mxConstants.STYLE_FONTFAMILY, mxConstants.DEFAULT_FONTFAMILY);
		var color = mxUtils.getValue(state.style, mxConstants.STYLE_TEXTCOLOR,
				'black');
		var align = (this.graph.model.isEdge(state.cell))
				? mxConstants.ALIGN_LEFT
				: mxUtils.getValue(state.style, mxConstants.STYLE_ALIGN,
						mxConstants.ALIGN_LEFT);
		var bold = (mxUtils.getValue(state.style, mxConstants.STYLE_FONTSTYLE,
				0) & mxConstants.FONT_BOLD) == mxConstants.FONT_BOLD;
...
mxCellRenderer.prototype.getLabelValue = function(state) {
	...
	if (!graph.isHtmlLabel(state.cell) && !mxUtils.isNode(value) && false
			&& value != null) {
...
mxCellRenderer.prototype.createLabel = function(state, value) {
	...
	if (state.style[mxConstants.STYLE_FONTSIZE] > 0
		...
		state.text = new mxText(
				...
				state.style[mxConstants.STYLE_LABEL_BORDERCOLOR], graph
						.isWrapping(state.cell), graph
...
mxCellRenderer.prototype.installCellOverlayListeners = function(state, overlay,
	...
	mxEvent.addListener(shape.node, 'click', function(evt) {
				overlay.fireEvent(new mxEventObject(mxEvent.CLICK, 'event',
						evt, 'cell', state.cell));
			});
...
var mxEdgeStyle = {
	EntityRelation : function(state, source, target, points, result) {
		...
		if (source != null && target != null) {
			...
			if (isSourceLeft == isTargetLeft) {
				var x = (isSourceLeft) ? Math.min(x0, xe) - segment : Math.max(
						x0, xe)
						+ segment;
	...
	SideToSide : function(state, source, target, points, result) {
		...
		if (source != null && target != null) {
			...
			if (pt != null) {
				if (p0 != null && pt.y >= source.y
						&& pt.y <= source.y + source.height) {
					y1 = pt.y;
				}
				if (pe != null && pt.y >= target.y
	...
	TopToBottom : function(state, source, target, points, result) {
		...
		if (source != null && target != null) {
			var t = Math.max(source.y, target.y);
			var b = Math
					.min(source.y + source.height, target.y + target.height);
			var x = view.getRoutingCenterX(source);
			if (pt != null && pt.x >= source.x
	...
	if (state != null) {
		if (state.invalid) {
			...
			if (model.isEdge(cell)) {
				...
				var target = this
						.getState(this.getVisibleTerminal(cell, false));
		...
		if (model.isEdge(cell) || model.isVertex(cell)) {
			...
			var box = (state.text != null && !this.graph
					.isLabelClipped(state.cell))
...
mxGraphView.prototype.updateFixedTerminalPoints = function(edge, source, target) {
	this.updateFixedTerminalPoint(edge, source, true, this.graph
					.getConnectionConstraint(edge, source, true));
	this.updateFixedTerminalPoint(edge, target, false, this.graph
					.getConnectionConstraint(edge, target, false));
};
mxGraphView.prototype.updateFixedTerminalPoint = function(edge, terminal,
		source, constraint) {
	var pt = null;
	if (constraint != null) {
...
mxGraphView.prototype.getEdgeStyle = function(edge, points, source, target) {
	var edgeStyle = (source != null && source == target)
			...
			: (!mxUtils.getValue(edge.style, mxConstants.STYLE_NOEDGESTYLE,
					false) ? edge.style[mxConstants.STYLE_EDGE] : null);
...
mxGraphView.prototype.updateFloatingTerminalPoints = function(state, source,
		target) {
	var pts = state.absolutePoints;
	var p0 = pts[0];
	var pe = pts[pts.length - 1];
...
mxGraphView.prototype.updateFloatingTerminalPoint = function(edge, start, end,
		source) {
	start = this.getTerminalPort(edge, start, source);
	var next = this.getNextPoint(edge, end, source);
	var border = parseFloat(edge.style[mxConstants.STYLE_PERIMETER_SPACING]
			|| 0);
	border += parseFloat(edge.style[(source)
			? mxConstants.STYLE_SOURCE_PERIMETER_SPACING
			: mxConstants.STYLE_TARGET_PERIMETER_SPACING]
			|| 0);
	var pt = this.getPerimeterPoint(start, next, this.graph.isOrthogonal(edge),
			border);
	edge.setAbsoluteTerminalPoint(pt, source);
};
...
mxGraphView.prototype.getPerimeterPoint = function(terminal, next, orthogonal,
		border) {
	var point = null;
	if (terminal != null) {
...
mxGraphView.prototype.createSvg = function() {
	...
	var onResize = mxUtils.bind(this, function(evt) {
				if (this.graph.container != null) {
					...
					root
							.setAttribute('height', Math.max(height,
											bounds.height));
				}
			});
...
mxGraph.prototype.containsValidationErrorsResource = (mxClient.language != 'none')
		...
		: '';
mxGraph.prototype.collapseExpandResource = (mxClient.language != 'none')
		? 'collapse-expand'
		: '';
...
mxGraph.prototype.sizeDidChange = function() {
	...
	if (this.container != null) {
		...
		if (this.preferPageSize || (!false && this.pageVisible)) {
			...
			var page = new mxRectangle(scale * tr.x, scale * tr.y, fmt.width
							* ps, fmt.height * ps);
			var hCount = (this.pageBreaksVisible) ? Math.ceil(width
					/ page.width) : 1;
			var vCount = (this.pageBreaksVisible) ? Math.ceil(height
					/ page.height) : 1;
...
mxGraph.prototype.updatePageBreaks = function(visible, width, height) {
	...
	var fmt = this.pageFormat;
	var ps = scale * this.pageScale;
	var bounds = new mxRectangle(scale * tr.x, scale * tr.y, fmt.width * ps,
			fmt.height * ps);
	visible = visible
	...
	var horizontalCount = (visible) ? Math.ceil((width - bounds.x)
			/ bounds.width) : 0;
	var verticalCount = (visible) ? Math.ceil((height - bounds.y)
			/ bounds.height) : 0;
	var right = width;
	var bottom = height;
	...
	if (this.horizontalPageBreaks != null) {
		...
		this.horizontalPageBreaks.splice(horizontalCount,
				this.horizontalPageBreaks.length - horizontalCount);
	}
	...
	if (this.verticalPageBreaks != null) {
		...
		this.verticalPageBreaks.splice(verticalCount,
				this.verticalPageBreaks.length - verticalCount);
	}
};
				...
				if (children != null && children.length > 0) {
					...
					if (childBounds.width > 0 && childBounds.height > 0) {
						...
						geo.height = childBounds.height + size.height + 2
								* border;
						this.model.setGeometry(cells[i], geo);
						this.moveCells(children, -childBounds.x + size.width
										+ border, -childBounds.y + size.height
										+ border);
					}
				}
			}
...
mxGraph.prototype.translateCell = function(cell, dx, dy) {
	...
	if (geo != null) {
		geo = geo.clone();
		geo.translate(dx, dy);
		if (!geo.relative && this.model.isVertex(cell)
				&& !this.isAllowNegativeCoordinates()) {
			geo.x = Math.max(0, geo.x);
			geo.y = Math.max(0, geo.y);
		}
...
mxGraph.prototype.setConnectionConstraint = function(edge, terminal, source,
		constraint) {
	if (constraint != null) {
		this.model.beginUpdate();
		try {
			if (constraint == null || constraint.point == null) {
				this.setCellStyles((source)
								? mxConstants.STYLE_EXIT_X
				...
				this.setCellStyles((source)
								? mxConstants.STYLE_EXIT_X
								: mxConstants.STYLE_ENTRY_X,
						constraint.point.x, [edge]);
				this.setCellStyles((source)
								? mxConstants.STYLE_EXIT_Y
								: mxConstants.STYLE_ENTRY_Y,
						constraint.point.y, [edge]);
...
mxGraph.prototype.getDropTarget = function(cells, evt, cell) {
	...
	return (!this.model.isLayer(cell) && mxUtils.indexOf(cells, cell) < 0)
			? cell
			: null;
};
...
mxGraph.prototype.getEdges = function(cell, parent, incoming, outgoing,
	...
	for (var i = 0; i < edges.length; i++) {
		...
		if (includeLoops
				|| ((source != target) && ((incoming && target == cell && (parent == null || this.model
...
mxOutline.prototype.update = function(revalidate) {
	...
	var unscaledGraphBounds = new mxRectangle(
			scaledGraphBounds.x / sourceScale, scaledGraphBounds.y
					/ sourceScale, scaledGraphBounds.width / sourceScale,
	...
	var completeWidth = Math.max(this.source.container.scrollWidth, union.width
					+ union.x);
	var completeHeight = Math.max(this.source.container.scrollHeight,
			union.height + union.y);
	var availableWidth = Math.max(0, this.outline.container.clientWidth
					- this.border);
	var availableHeight = Math.max(0, this.outline.container.clientHeight
					- this.border);
	var outlineScale = Math.min(availableWidth / completeWidth, availableHeight
					/ completeHeight);
...
mxSpaceManager.prototype.getCellsToShift = function(state) {
	...
	return graph
			.getCellsBeyond(state.x + ((down) ? 0 : state.width), state.y
							+ ((down && right) ? 0 : state.height), parent,
					right, down);
};
...
mxCellStatePreview.prototype.moveState = function(state, dx, dy, add,
		includeEdges) {
...
mxCellStatePreview.prototype.translateState = function(parentState, state, dx,
		dy) {
	if (state != null) {
		var model = this.graph.getModel();
		...
		for (var i = 0; i < childCount; i++) {
			this.translateState(state, this.graph.view.getState(model
							.getChildAt(state.cell, i)), dx, dy);
		}
	}
};
mxCellStatePreview.prototype.revalidateState = function(parentState, state, dx,
		dy, visitor) {
	if (state != null) {
		state.invalid = true;
		...
		var model = this.graph.getModel();
		var geo = model.getGeometry(state.cell);
		if (geo != null && dx != 0 && dy != 0 && model.isVertex(state.cell)
				&& geo.relative && this.deltas[id] != null) {
			state.x += dx;
			state.y += dy;
		...
		var childCount = model.getChildCount(state.cell);
		for (var i = 0; i < childCount; i++) {
			this.revalidateState(state, graph.view.getState(model.getChildAt(
...
mxGraphHandler.prototype.mouseMove = function(sender, me) {
	...
	if (!me.isConsumed() && graph.isMouseDown && this.cell != null
		...
		me.consume();
			...
			&& this.updateCursor && !me.isConsumed() && me.getState() != null
			&& !graph.isMouseDown) {
...
mxConnectionHandler.prototype.createMarker = function() {
	...
	marker.getCell = mxUtils.bind(this, function(evt, cell) {
				var cell = mxCellMarker.prototype.getCell
	...
	marker.getMarkerColor = mxUtils.bind(this, function(evt, state, isValid) {
				return (this.connectImage == null || this.isConnecting())
						? mxCellMarker.prototype.getMarkerColor.apply(marker,
								arguments)
						: null;
			});
	marker.intersects = mxUtils.bind(this, function(state, evt) {
				...
				return mxCellMarker.prototype.intersects.apply(marker,
						arguments);
			});
	return marker;
};
	...
	if (image != null && state != null) {
		...
		if (this.moveIconFront) {
			...
			if (this.moveIconBack && icon.node.previousSibling != null) {
				icon.node.parentNode.insertBefore(icon.node,
...
mxConnectionHandler.prototype.mouseMove = function(sender, me) {
	if (!me.isConsumed()
		...
		if (this.previous != null && this.start != null) {
			...
			if (state != null) {
				if (this.constraintHandler.currentConstraint == null) {
					...
					if (targetPerimeter != null) {
						...
						var tmp = targetPerimeter(view
										.getPerimeterBounds(state),
			...
			if (this.sourceConstraint == null) {
				...
				if (sourcePerimeter != null) {
					var tmp = sourcePerimeter(view
									.getPerimeterBounds(this.previous),
							this.previous, current, false);
					if (tmp != null) {
						pt2 = tmp;
					}
				} else {
					pt2 = new mxPoint(this.previous.getCenterX(), this.previous
									.getCenterY());
				}
			}
			if (state == null) {
			...
			this.previous = state;
		} else if (this.previous == state && state != null
				&& this.icons == null && !this.graph.isMouseDown) {
			me.consume();
		}
		if (this.constraintHandler.currentConstraint != null) {
			this.marker.reset();
		...
		if (!this.graph.isMouseDown && state != null && this.icons != null) {
			var checkBounds = state.text != null
...
mxConnectionHandler.prototype.connect = function(source, target, evt,
	...
	if (source != null
		...
		try {
			...
			if (edge != null) {
				this.graph.setConnectionConstraint(edge, source, true,
...
mxConnectionHandler.prototype.createTargetVertex = function(evt, source) {
	...
	if (geo != null) {
		...
		if (this.start != null) {
			...
			if (sourceState != null) {
				var tol = (this.graph.isGridEnabled())
						? this.graph.gridSize
						: this.graph.tolerance;
				if (Math.abs(this.graph.snap(this.start.x)
						- this.graph.snap(point.x)) <= tol) {
					geo.x = sourceState.x;
				} else if (Math.abs(this.graph.snap(this.start.y)
...
mxConstraintHandler.prototype.update = function(me) {
	...
	if (this.currentFocusArea == null
			|| !mxUtils.intersects(this.currentFocusArea, mouse)) {
		this.currentFocusArea = null;
		if (me.getState() != this.currentFocus) {
			this.currentFocus = me.getState();
			if (me.getState() != null) {
				this.constraints = this.graph.getAllConnectionConstraints(me
						.getState());
				if (this.constraints != null) {
					this.currentFocusArea = new mxRectangle(me.getState().x, me
									.getState().y, me.getState().width, me
									.getState().height);
					if (this.focusIcons != null) {
						for (var i = 0; i < this.focusIcons.length; i++) {
							this.focusIcons[i].destroy();
					...
					for (var i = 0; i < this.constraints.length; i++) {
						var cp = this.graph.getConnectionPoint(me.getState(),
								this.constraints[i]);
						...
						mxEvent.redirectMouseEvents(icon.node, this.graph,
								getState);
		...
		if (this.graph.isCellResizable(this.state.cell)) {
			...
			if (geo != null && !geo.relative
				...
				this.labelShape = this.createSizer(
						mxConstants.CURSOR_LABEL_HANDLE, mxEvent.LABEL_HANDLE,
...
mxEdgeHandler.prototype.init = function() {
	...
	this.preferHtml = this.state.text != null
			&& this.state.text.node.parentNode == this.graph.container;
	if (!this.preferHtml) {
		var sourceState = this.graph.view.getState(this.graph.view
				.getVisibleTerminal(this.state.cell, true));
		if (sourceState != null) {
			this.preferHtml = sourceState.text != null
					&& sourceState.text.node.parentNode == this.graph.container;
		}
		if (!this.preferHtml) {
			var targetState = this.graph.view.getState(this.graph.view
					.getVisibleTerminal(this.state.cell, false));
			if (targetState != null) {
		...
		if (this.isHandleVisible(i)) {
			...
			if (terminal || this.graph.isCellBendable(cell)) {
				...
				if (this.isHandleEnabled(i)) {
					...
					mxEvent.redirectMouseEvents(bend.node, this.graph,
							this.state);
				}
				bends.push(bend);
				if (!terminal) {
...
mxEdgeHandler.prototype.mouseDown = function(sender, me) {
	...
	if (!me.isConsumed() && this.graph.isEnabled()
			&& !this.graph.isForceMarqueeEvent(me.getEvent()) && handle != null) {
		if (this.removeEnabled && this.isRemovePointEvent(me.getEvent())) {
			this.removePoint(this.state, handle);
		} else if (handle != mxEvent.LABEL_HANDLE
...
mxEdgeHandler.prototype.mouseMove = function(sender, me) {
	if (this.index != null && this.marker != null) {
		...
		if (this.isLabel) {
			...
			var sourceConstraint = this.graph.getConnectionConstraint(clone,
					sourceState, true);
			var targetConstraint = this.graph.getConnectionConstraint(clone,
					targetState, false);
			...
			if (!this.isSource || sourceState != null) {
				clone.view.updateFixedTerminalPoint(clone, sourceState, true,
						sourceConstraint);
			}
			if (!this.isTarget || targetState != null) {
				clone.view.updateFixedTerminalPoint(clone, targetState, false,
						targetConstraint);
			}
			clone.view.updatePoints(clone, points, sourceState, targetState);
			clone.view.updateFloatingTerminalPoints(clone, sourceState,
					targetState);
	...
	if (this.index != null && this.marker != null) {
		...
		if (me.getX() != this.startX || me.getY() != this.startY) {
			if (this.error != null) {
				...
				if (terminal != null) {
					var edge = this.connect(edge, terminal, this.isSource,
...
mxEdgeHandler.prototype.changeTerminalPoint = function(edge, point, isSource) {
	...
	if (geo != null) {
		...
		try {
			...
			model.setGeometry(edge, geo);
			this.graph.connectCell(edge, null, isSource,
...
mxEdgeHandler.prototype.redraw = function() {
	...
	this.label = new mxPoint(this.state.absoluteOffset.x,
			this.state.absoluteOffset.y);
	this.labelShape.bounds = new mxRectangle(this.label.x - s / 2, this.label.y
					- s / 2, s, s);
	...
	if (pts != null) {
		for (var i = 1; i < this.bends.length - 1; i++) {
			if (this.abspoints[i] != null) {
				...
				this.bends[i].bounds = new mxRectangle(x - s / 2, y - s / 2, s,
						s);
				this.bends[i].redraw();
...
mxEdgeHandler.prototype.drawPreview = function() {
	if (this.isLabel) {
		var s = mxConstants.LABEL_HANDLE_SIZE;
		var bounds = new mxRectangle(this.label.x - s / 2,
...
mxElbowEdgeHandler.prototype.createVirtualBend = function() {
	...
	mxEvent.redirectMouseEvents(bend.node, this.graph, this.state, null, null,
			null, dblClick);
...
mxTooltipHandler.prototype.mouseMove = function(sender, me) {
	if (me.getX() != this.lastX || me.getY() != this.lastY) {
		...
		if (this.isHideOnHover()
				|| me.getState() != this.state
				|| (me.getSource() != this.node && (!this.stateSource || (me
						.getState() != null && this.stateSource == (me
						.isSource(me.getState().shape) || !me.isSource(me
						.getState().text)))))) {
			this.hide();
		}
	}
	...
	if (restart && this.isEnabled() && me.getState() != null
		...
		this.thread = window.setTimeout(mxUtils.bind(this, function() {
							if (!this.graph.isEditing()
									&& !this.graph.panningHandler
											.isMenuShowing()) {
								var tip = this.graph.getTooltip(state, node, x,
										y);
								this.show(tip, x, y);
								this.state = state;
								this.node = node;
...
mxCellHighlight.prototype.createShape = function(state) {
	...
	if (this.graph.model.isEdge(state.cell)) {
		shape = new mxPolyline(state.absolutePoints, this.highlightColor,
				this.strokeWidth);
	} else {
		shape = new mxRectangleShape(new mxRectangle(state.x - 2, state.y - 2,
						state.width + 4, state.height + 4), null,
...
mxEditor.prototype.createSession = function() {
	...
	var sessionChanged = mxUtils.bind(this, function(session) {
				this.fireEvent(new mxEventObject(mxEvent.SESSION, 'session',
						session));
			});
...
mxEditor.prototype.installInsertHandler = function(graph) {
	...
	var insertHandler = {
		mouseDown : function(sender, me) {
			if (self.insertFunction != null && !me.isPopupTrigger()
