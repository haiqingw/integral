...
#
# Last modified: 2016-03-04 14:21
#
# Filename: Alipay.class.php
#
# Description: 支付宝Api
#
=============================================================================*/
class DConfig{
    ...
    static function get($key){
        $config = array (
            'alipay_public_key_file' => getcwd().DIRECTORY_SEPARATOR.'ThinkPHP\Library\Vendor\key'.DIRECTORY_SEPARATOR."alipay_rsa_public_key.pem", //支付宝公钥
            'merchant_private_key_file' => getcwd().DIRECTORY_SEPARATOR.'ThinkPHP\Library\Vendor\key'.DIRECTORY_SEPARATOR."rsa_private_key.pem",    //RSA私钥
            'merchant_public_key_file' => getcwd().DIRECTORY_SEPARATOR.'ThinkPHP\Library\Vendor\key'.DIRECTORY_SEPARATOR."rsa_public_key.pem",	    //RSA公钥	
            'charset' => "UTF-8",    //编码
            'gatewayUrl' => "https://openapi.alipay.com/gateway.do",    //支付宝网关
            'app_id' => "2016030201176669"  //支付宝应用id
        );
        return $config[$key];
    }
    /**
    ...
    static function writeLog($text) {
        ...
        file_put_contents ( "./Uploads/log/zfb_log.txt", date ( "Y-m-d H:i:s" ) . "  " . $text . "\r\n", FILE_APPEND );
    }
...
class Alipay{
    ...
    private $notify_url = "http://pay.xylrcs.cn/alipay/notify.php";

    //给默认参数赋值
    public function setParams($key,$val){
        $this->paramsQrpay[$key] = $val;
    }

    //获取订单号
    public function get_out_trade_no($num = ""){
        $ordernum = empty($num)?"YDSM_".date('YmdHis').DConfig::random_str(4).mt_rand(1000,9999):$num;
        $this->paramsQrpay['out_trade_no'] = $ordernum;
        return $ordernum;
    }
    ...
    //调用支付宝扫码支付
    public function callQrpay($token = NULL){
        $biz_content = json_encode($this->paramsQrpay);
        //echo $biz_content;
		$request = new AlipayTradePrecreateRequest();
        $request->setNotifyUrl($this->notify_url);
		$request->setBizContent ( $biz_content );
        $aop = new AopClient ();
        $aop->gatewayUrl = DConfig::get('gatewayUrl');
        $aop->appId = DConfig::get('app_id');
        $aop->rsaPrivateKeyFilePath = DConfig::get('merchant_private_key_file');
        $aop->apiVersion = "1.0";
        $result = $aop->execute ( $request, $token );
        DConfig::writeLog("response: ".var_export($result,true));
        $res = get_object_vars($result->alipay_trade_precreate_response);
        if($res['msg']=="Success"){
            return $res['qr_code'];
        }else{
            return false;
...
/*
$alipay=new Alipay;
echo $alipay->get_out_trade_no();
print_r($alipay->callQrpay());
 */
