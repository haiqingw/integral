...
class traverseDir{
    ...
    public function tozipNew($items){ 
        $zip=new ZipArchive();
        $zipname=date('YmdHis',time());
        if (!file_exists($zipname)){
            $zip->open($this->savepath.$zipname.'.zip',ZipArchive::CREATE|ZipArchive::OVERWRITE);//创建一个空的zip文件
            for ($i=0;$i<count($items);$i++){
...
class download{
    ...
    public function getfiles(){
        ...
        if (file_exists($this->_filepath)){
            ...
            echo "<script>alert('对不起,您要下载的文件不存在');window.history.back();</script>";
        }
    }
}
