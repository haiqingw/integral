...
function navTabAjaxDone(json){
	...
	if (json[DWZ.keys.statusCode] == DWZ.statusCode.ok){
		...
		} else if ("dialogReload" == json.callbackType){ //2016-08-10 hojk 添加 作用:刷新局部dialog
			$.pdialog.reload(json.forwardUrl, {data:{}, dialogId:json.navTabId, callback:null}) 
...
function uploadifySuccess(file, data, response){
	var $datas = eval('(' + data + ')');
	upimg.push($datas.Img); //向数组最后插入记录
	$('#upimg').val(upimg);
}
