...
(function($){
	if ($.validator) {
		...
		//自定身份证号码验证
		$.validator.addMethod("IDRegular",function(value,element){
			return this.optional(element) || /^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|X)$/.test(value);
		},"Please specify a valid identity card number");
		...
		return this.each(function(){
			...
			$this.click(function(event){
                ...
                $(setting.todayBut$).click(function(){
					...
					//$this.val(todayDate.getFullYear()+'-'+todayDate.getMonth()+'-'+todayDate.getDate());
					closeCalendar();
				});
