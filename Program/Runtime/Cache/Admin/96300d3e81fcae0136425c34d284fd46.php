    ...
    <form id="pagerForm" onsubmit="return divSearch(this, 'jbsxBoxSYSZXLB');" action="<?php echo U('POSstatistics/translists');?>"
        ...
        <div class="searchBar">
            <table class="searchContent">
                <tr>
                    <td align="left">交易日期： <input type="text" name="sdate" style="width: 80px;" class="date" readonly="true"
                            datefmt="yyyy-MM-dd" value="<?php echo ($sdate); ?>" /> ~ <input type="text" name="edate" style="width: 80px;"
                            class="date" readonly="true" datefmt="yyyy-MM-dd" value="<?php echo ($edate); ?>" /></td>
                    <td align="right">关键词：<input type="text" value="<?php echo ($keywords); ?>" id="keywords" name="keywords"
                            autocomplete="off" placeholder="订单号,终端号,交易金额" style="width: 160px;" />
                    </td>
...
<div class="pageContent" style="border-left:1px #B8D0D6 solid;border-right:1px #B8D0D6 solid;width:100%;">
    ...
    <table class="table" width="99%" layoutH="186" rel="jbsxBoxSYSZXLB">
        ...
        <?php if($totalCount > 0): ?><tbody>
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr target="bid" rel="<?php echo ($v["id"]); ?>">
                        ...
                        <td align="center"><?php echo ($v["terminalNoNo"]); ?></td>
                        ...
                        <td align="center"><?php echo ($v["tradeResult"]); ?></td>
