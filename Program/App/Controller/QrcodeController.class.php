...
class QrcodeController extends BaseController{
	public function index($bid){
		$row = fRec(T_BUS,"id=" . $bid,"busname,plat");
		$filePath = self::getPath($bid);
		$headimg = "./Public/sft/images/logo.jpg";
		$qrcode = $filePath . "qrcode.png";
		$bgimg = $filePath . "bgimg.jpg";
		//$par = parent::encode("pid:{$row['plat']}|tid:{$bid}");
		//$text = BASEURL . "/index.php/Home/Register/index/bid/" . $bid . "/resType/SM";
		$code = parent::encode("bid:".$bid.",resType:SM");
		$text = BASEURL . "/index.php/Home/Register/index?code=".$code; 
		// if(!is_file($qrcode)){
			$this->createImg($bid,$headimg,$text);
		// }
		//if(!is_file($bgimg)){
			$text = "我是" . $row['busname'] . "，我为刷多宝代言";
			$this->mergedImages($qrcode,$bgimg,$text,$row['plat']);
		//}
	 ...
	 * 获取背景图片
	 */
	protected static function getBackPic($plat){
		if(empty($plat)){
			return false;
		}
		$where = array("plat" => $plat, "status" => 1, "useStatus" => 1);
		$info = M(T_LA)->field(array("picUrl", "xAxis", "yAxis", "qrSize"))->where($where)->find();
		if($info){
			return $info;
		}else{
			return false;
	...
	//合并二维码和背景
	protected function mergedImages($qr, $savePath, $text = "", $pid){
		// 		$first = "./Public/sft/images/codeImg.jpg";
		// 		$x = 160;
		// 		$y = 600;
		// 		$qrSize = "430";
		$first = "./Public/images/defaultDiagramImg.jpg";
		$x = 229;
		$y = 398;
		$qrSize = 295;
		$background = self::getBackPic($pid);
		if($background){
			$first = "." . $background['picUrl'];
			$x = $background['xAxis'];
			$y = $background['yAxis'];
			$qrSize = $background['qrSize'];
		}
		$headSize = "55";
	...
	protected static function getPath($openid){
		$filePath = array("./Uploads/", "shareQrcode/", md5($openid) . "/");
